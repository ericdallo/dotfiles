...
iw phy $PHY_OF_WLAN_1 interface add mon1 type monitor 2> /dev/null;
iw phy $PHY_OF_WLAN_1 interface add mon2 type monitor 2> /dev/null;
iw phy $PHY_OF_WLAN_1 interface add mon3 type monitor 2> /dev/null;
...
iw dev mon1 set monitor none
iw dev mon2 set monitor none
iw dev mon3 set monitor none
...
function cleanup
{
    killall -9 ReVdK3-r3.sh 2&>/dev/null;
    killall -9 mdk3 2&> /dev/null;
    killall -9 reaver 2&> /dev/null;
    killall -9 bully 2&> /dev/null;
    killall tail 2&> /dev/null;
    rm -f /etc/reaver_tmp.txt 2&> /dev/null;
    rm -f /etc/bully_tmp.txt 2&> /dev/null;

    iw dev mon1 del 2&> /dev/null;
    iw dev mon2 del 2&> /dev/null;
    iw dev mon3 del 2&> /dev/null;

    ps aux | grep 'ReVdK3' | awk '{print $2}' | xargs -i  kill -9 2&>/dev/null

    for pid in `ps aux| grep ReVdK3 | awk '{print $2}'`; do
                kill -9 $pid 2&>/dev/null;
            done

            killall aireplay-ng 2&> /dev/null;
            rm -f /etc/aireplay_tmp.txt 2&> /dev/null;
            killall -9 ReVdK3-r3.sh 2&> /dev/null;
            clear
        }
